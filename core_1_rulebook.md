...
### 試合
タイマーのカウントダウンとともに、2分間の試合が開始する。試合中、迎撃サイドの操縦手はヘッドセットを使って相互に会話することができる。試合が終了した後、各チームの整備手はロボットの電源を切るか、非常停止スイッチを操作しなければならない。
...
## 決勝トーナメント
決勝トーナメントでは24m×15mのフィールドを使用する（[3DCAD図はこちらを参照](https://a360.co/3ztLLhT)）。以下の図ではフィールド形状をわかりやすくするため、青サイド側のみにブロックを配置している。
