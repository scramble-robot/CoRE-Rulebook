...
### 撃破からの復活
ロボットが撃破されてから45秒が経過すると、当該ロボットのHPが60に自動回復して復活し、その場から動作を再開できる。ただし、当該ロボットが転倒状態にある場合は復活できない。なお、復活から5秒間はHPが減少しない無敵状態とする。
...
### 失格
審判が以下の失格行為を認めた場合、総大将決定戦の場合は失格行為を行ったチームのロボットを撃破した状態で試合を続行し、当該チームの順位は最下位とする。決勝戦の場合、当該ラウンドは相手同盟の勝利とする。
