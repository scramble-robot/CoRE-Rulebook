...
**⚠注意**
- ルールへの意見、疑問点などはすべてCoRE-RulebookのGithubリポジトリ内のissueに記入すること。記入方法は後日詳細を発表する。
- 実行委員会への直接的なルールへの意見、質問は原則一切受け付けない。ただし、個人情報など公開できない事柄が含まれる場合などは、これに限らない。

CoREでは、参加者自らがルールのあるべき姿を、参加者相互に議論してルールを見直し、より良いルールを作り上げるという観点から、ルールメイキングを実施する。ルールメイキングの方法については後日詳細を発表する。

なお、ルールメイキングの対象となるのは、以下の項目を予定している。
- 共通ルールブック
    - 5章　操縦ロボットの規則
    - 6章　安全管理の規則
- 1部リーグルールブック
